...
import { EmptyUnitsSvg } from "@/components/portfolio/empty-illustrations";
...
import {
  UNIT_COLUMN_STORAGE_KEY,
  defaultUnitVisibleColumns,
  unitTableColumns,
} from "@/components/portfolio/unit-table-config";
import { UNIT_STATUSES, UNIT_TYPES } from "@/components/portfolio/unit-form";
...
import { deleteUnit, listUnits } from "@/services/unit-service";
import type { PaginatedResponse } from "@/types/api";
import type { UnitDto } from "@/types/portfolio";

const TABLE_PAGE_SIZE = 50;
const GRID_PAGE_SIZE = 24;
const VIEW_KEY = "portfolio-view-units";
...
function UnitsPageContent() {
  ...
  const [buildingMap, setBuildingMap] = useState<Map<number, string>>(
    new Map(),
  );
  const [buildingCount, setBuildingCount] = useState(0);
  const [buildingMetaLoading, setBuildingMetaLoading] = useState(true);

  const [search, setSearch] = useState(() => searchParams.get("search") ?? "");
  const [status, setStatus] = useState(() => searchParams.get("status") ?? "");
  const [unitType, setUnitType] = useState(
    () => searchParams.get("unit_type") ?? "",
  );
  ...
  const [filtersOpen, setFiltersOpen] = useState(() => {
    return !!(
      ...
      (searchParams.get("unit_type") ?? "").trim() ||
  ...
  useEffect(() => {
    ...
    setUnitType(searchParams.get("unit_type") ?? "");
    setBuildingId(searchParams.get("building") ?? "");
  ...
  useEffect(() => {
    ...
    if (unitType) next.set("unit_type", unitType);
    if (bid !== "" && Number.isFinite(Number(bid))) next.set("building", bid);
  ...
  const listParams = useMemo(() => {
    ...
    return {
      ...
      building:
        bid !== "" && Number.isFinite(Number(bid)) ? Number(bid) : undefined,
      ordering: ordering || undefined,
    };
  }, [debouncedSearch, status, unitType, buildingId, ordering]);
  ...
  const hasActiveFilters = useMemo(
    () =>
      ...
      ordering !== "",
    [search, status, unitType, buildingId, ordering],
  );
  ...
  const [tableData, setTableData] =
    useState<PaginatedResponse<UnitDto> | null>(null);
  ...
  const columns = useMemo(
    () =>
      unitTableColumns((id) => buildingMap.get(id) ?? `Building #${id}`),
    [buildingMap],
  );

  const [gridItems, setGridItems] = useState<UnitDto[]>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setBuildingMetaLoading(true);
    (async () => {
      try {
        const all = await listAllBuildings();
        if (cancelled) return;
        const m = new Map<number, string>();
        for (const b of all) m.set(b.id, b.name);
        setBuildingMap(m);
        setBuildingCount(all.length);
      } catch {
        if (!cancelled) {
        ...
        if (!cancelled) setBuildingMetaLoading(false);
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setLoadError(
            e instanceof ApiError ? e.messageForUser : "Could not load units.",
          );
          setTableData(null);
        }
      } finally {
  ...
  if (!orgReady) {
    return (
      <DashboardListView title="Units" description="Loading…">
        <div className="text-sm text-[#6B7280]">Preparing workspace…</div>
      </DashboardListView>
    );
  }

  const canAddUnit = orgId != null && buildingCount > 0;

  return (
    <>
      <DashboardListView
        title="Units"
        description="Rentable spaces inside buildings — foundation for leases and rent roll."
        actions={
          <div className="flex flex-wrap items-center gap-3">
            <PortfolioViewToggle
              mode={viewMode}
              onChange={setViewMode}
              testId="units-view-toggle"
            />
            {canAddUnit ? (
              <Link
                href="/dashboard/units/create"
                className="btn-primary-sm"
                data-testid="units-add"
              >
                Add unit
              </Link>
            ) : (
      ...
      >
        ...
        {orgId != null && !buildingMetaLoading ? (
            ...
            {searchOpen ? (
              <div className="mt-3">
                <input
                  ...
                  placeholder="Search unit #, floor, notes…"
                  value={search}
        ...
        !loadError ? (
          <PortfolioEmptyState
            illustration={<EmptyUnitsSvg className="w-full" />}
            title="Add a building first"
            description="Units belong to a building. Create landlords, then buildings, then add units here."
            primaryAction={
              <Link href="/dashboard/buildings" className="btn-primary">
                Go to buildings
              </Link>
            }
          />
        ...
        !loadError ? (
          <PortfolioEmptyState
            illustration={<EmptyUnitsSvg className="w-full" />}
            title="No units yet"
            description="Add each suite or door so you can track occupancy, leases, and billing per space."
            primaryAction={
              <Link href="/dashboard/units/create" className="btn-primary">
                Add your first unit
              </Link>
            }
            secondaryAction={
          ...
          <div className="overflow-hidden rounded-xl border border-[#E5E7EB] bg-white shadow-sm">
            <div className="overflow-x-auto">
              <PortfolioDataTable<UnitDto>
                ...
                onSort={onSort}
                onRowClick={(row) =>
                  router.push(`/dashboard/units/${row.id}`)
                }
                renderActions={(row) => (
                  <>
                    <Link
                      href={`/dashboard/units/${row.id}/edit`}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-[#6B7280]">
                          {buildingMap.get(row.building) ??
                            `Building #${row.building}`}
                        </p>
                        <p className="mt-2 text-xs text-[#6B7280]">
                          {row.unit_type} · {row.status}
                        </p>
                        {row.floor ? (
                          <p className="mt-1 text-xs text-[#6B7280]">
                            Floor {row.floor}
      ...
      <ConfirmDeleteDialog
        ...
        title="Delete unit?"
        message={`Remove unit “${deleteTarget?.unit_number}”? Active leases may prevent deletion.`}
