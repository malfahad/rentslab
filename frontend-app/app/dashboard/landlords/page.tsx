...
import { EmptyLandlordsSvg } from "@/components/portfolio/empty-illustrations";
...
import {
  LANDLORD_COLUMN_STORAGE_KEY,
  LANDLORD_TABLE_COLUMNS,
  defaultLandlordVisibleColumns,
} from "@/components/portfolio/landlord-table-config";
...
import { deleteLandlord, listLandlords } from "@/services/landlord-service";
import type { PaginatedResponse } from "@/types/api";
import type { LandlordDto } from "@/types/portfolio";

const TABLE_PAGE_SIZE = 50;
const GRID_PAGE_SIZE = 24;
const VIEW_KEY = "portfolio-view-landlords";
...
function LandlordsPageContent() {
  ...
  const [tableData, setTableData] =
    useState<PaginatedResponse<LandlordDto> | null>(null);
  ...
  const [gridItems, setGridItems] = useState<LandlordDto[]>([]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<LandlordDto | null>(null);
  ...
  const columnPickerLabels = LANDLORD_TABLE_COLUMNS.map((c) => ({
    id: c.id,
    label: c.label,
  }));
  ...
  if (!orgReady) {
    return (
      <DashboardListView title="Landlords" description="Loading…">
  ...
  return (
    <>
      <DashboardListView
        title="Landlords"
        description="Property owners in your portfolio. Add buildings under each landlord."
        actions={
          <div className="flex flex-wrap items-center gap-3">
            <PortfolioViewToggle
              mode={viewMode}
              onChange={setViewMode}
              testId="landlords-view-toggle"
            />
            <Link
              href="/dashboard/landlords/create"
              className="btn-primary-sm"
              data-testid="landlords-add"
            >
              Add landlord
            </Link>
          </div>
      ...
      >
          ...
          <div className="mb-4 rounded-xl border border-[#E5E7EB] bg-white p-3 shadow-sm">
            <div className="flex items-center justify-end gap-0.5">
              <PortfolioToolbarIconButton
                ...
                testId="landlords-search-toggle"
              >
                <IconSearch />
              </PortfolioToolbarIconButton>
              <PortfolioToolbarIconButton
                ...
                testId="landlords-filters-toggle"
              >
                <IconFilter />
              </PortfolioToolbarIconButton>
              {hasActiveFilters ? (
                <PortfolioToolbarIconButton
                  ...
                  testId="landlords-clear-filters"
                >
                  <IconClearFilters />
                </PortfolioToolbarIconButton>
              ) : null}
              {viewMode === "table" ? (
                <PortfolioColumnPicker
                  ...
                  testId="landlords-column-picker"
                />
              ) : null}
            </div>
            {searchOpen ? (
        ...
        !loadError ? (
          <PortfolioEmptyState
            illustration={<EmptyLandlordsSvg className="w-full" />}
            title="No landlords yet"
            description="Create your first landlord to group buildings and keep ownership clear for reporting and compliance."
            primaryAction={
        ...
        {orgId != null && viewMode === "table" && (tableLoading || tableRows.length > 0) ? (
          <div className="overflow-hidden rounded-xl border border-[#E5E7EB] bg-white shadow-sm">
            <div className="overflow-x-auto">
              <PortfolioDataTable<LandlordDto>
                columns={LANDLORD_TABLE_COLUMNS}
                ...
                onSort={onSort}
                onRowClick={(row) =>
                  router.push(`/dashboard/landlords/${row.id}`)
                }
                renderActions={(row) => (
                  <>
                    <Link
                      href={`/dashboard/landlords/${row.id}/edit`}
      ...
      <ConfirmDeleteDialog
        ...
        message={`Remove “${deleteTarget?.name}” from this organization? Buildings must be reassigned or removed first if the API requires it.`}
