...
export function ExpenseForm({
  ...
  useEffect(() => {
    if (variant !== "edit" || !initialExpense) return;
    const e = initialExpense;
    setCategoryId(String(e.expense_category));
    const d = e.expense_date;
    setExpenseDate(d.length >= 10 ? d.slice(0, 10) : d);
    setAmount(e.amount);
    setDescription(e.description);
    setStatus(e.status);
    setBuildingId(e.building != null ? String(e.building) : "");
    setUnitId(e.unit != null ? String(e.unit) : "");
    setLeaseId(e.lease != null ? String(e.lease) : "");
    setVendorId(e.vendor != null ? String(e.vendor) : "");
    setJobOrderId(e.job_order != null ? String(e.job_order) : "");
    setPaymentMethod(e.payment_method || "");
    setReference(e.reference || "");
    setReceiptUrl(e.receipt_url || "");
    setExpenseNumber(e.expense_number || "");
    setCurrencyCode(e.currency_code || "");
    if (e.paid_at) {
      setPaidAtLocal(isoToDatetimeLocal(e.paid_at));
    } else {
      setPaidAtLocal(localDateTimeInputValue(new Date()));
    }
  }, [variant, initialExpense]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (expenseId == null || !Number.isFinite(expenseId)) {
        setFormError("Missing expense id.");
        return;
      }
      const patch: ExpenseUpdate = {
        ...
        building: bid != null && Number.isFinite(bid) ? bid : null,
        unit: uid != null && Number.isFinite(uid) ? uid : null,
        lease: lid != null && Number.isFinite(lid) ? lid : null,
        vendor: vid != null && Number.isFinite(vid) ? vid : null,
        job_order: joid != null && Number.isFinite(joid) ? joid : null,
        paid_at:
          status === "paid" ? new Date(paidAtLocal).toISOString() : null,
      };
      const updated = await updateExpense(expenseId, patch);
      onSuccess(updated);
    } catch (err) {
      setFormError(
      ...
      footer={
        <>
          ...
          <button
            ...
            disabled={pending}
          >
            {pending ? "Saving…" : variant === "edit" ? "Save changes" : "Save expense"}
          </button>
        </>
      }
    >
